...
def addInvitation(request):
    if request.method == 'POST':
        ...
        # 添加到数据库
        invi = invitation(number = number,name = name,grade = grade,sex = sex)
        invi.save()
...
def updateInvitation(request):
    if request.method == 'POST':
        ...
        grade = request.POST.get("grade", None)
        sex = request.POST.get("sex", None)
        invi = invitation.objects.filter(number=number).update(name=name,grade=grade,sex=sex)
    return HttpResponseRedirect('/index/')
...
def toquery(request):
    if request.method == 'POST':
        ...
        grade  = request.POST.get("grade",None)
        sex    = request.POST.get("sex",None)
    invi1 = invitation.objects.filter(sex=sex)
    invi2 = invitation.objects.filter(number=number)
    iniv3 = invitation.objects.filter(grade=grade)
    iniv4 = invitation.objects.filter(name = name)

    invi5 = invi1 or invi2 or iniv3 or iniv4
